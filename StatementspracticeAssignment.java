...
public class StatementspracticeAssignment {
    ...
    public static void main(String[] args) {
        ...
        if (name.strip().length() == 0) {
            System.out.printf("%sInvalid name Input%s \n", COLOR_RED, RESET);
            return;
            ...
            scanner.nextLine();
            if (age < 10 | age > 18) {
                System.out.printf("%sInvalid age Input%s \n", COLOR_RED, RESET);
                System.exit(0);
            } else {
                ...
                if (!((subject1.startsWith("SE-") | subject1.startsWith("se-")) & subject1.strip().length() > 3)) {
                    ...
                    if (subject1.equalsIgnoreCase(subject2)) {
                        System.out.printf("%sSubject is already entered%s \n", COLOR_RED, RESET);
                        System.exit(0);
                    } else {
                        if (!((subject2.startsWith("SE-") | subject2.startsWith("se-"))
                                & subject2.strip().length() > 3)) {
                            System.out.printf("%sInvalid Subject Input%s \n", COLOR_RED, RESET);
                            System.exit(0);
                        } else {
                            ...
                            if (subject3.equalsIgnoreCase(subject1) | subject3.equalsIgnoreCase(subject2)) {
                                System.out.printf("%sSubject is already entered%s \n", COLOR_RED, RESET);
                                System.exit(0);
                            } else {
                                if (!((subject3.startsWith("SE-") | subject3.startsWith("se-"))
                                        & subject3.strip().length() > 3)) {
                                    System.out.printf("%sInvalid Subject Input%s \n", COLOR_RED, RESET);
                                    System.exit(0);
                                } else {
